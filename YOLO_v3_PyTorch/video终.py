    ...
    def write(x, results, i):
        ...
        if label == 'blind tracks':
            ...
            if i > 200:
                ...
                if int(np.max(X[i - 10:i])) - int(np.min(X[i - 10:i])) < 500:
                ...
                if i <= 10:
                    if int(np.max(X)) - int(np.min(X)) < 500:
            ...
            if c3 < 700 and c4<V["blind"]["X"] and c5>V["blind"]["X"] and V["notBlind"]["bool"]==1:
                if label=='Person':
                    ...
                    # telegraph_pole.play()
                V["notBlind"]["bool"]=0
            elif c3 > 700 or c4>V["blind"]["X"] or c5>V["blind"]["X"] and V["notBlind"]["bool"]==0 and i%5==0:
                V["notBlind"]["bool"]=1
        return img
